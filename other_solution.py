...
for solution in solutions:
  ...
  if 'mine_L12_decoded.txt' not in filesInSolutionDir: continue
  with open(f'./mine/L12/solutions/{solution}/mine_L12_decoded.txt', 'r') as file:
    lines = file.readlines()[:-2]
    schedule = ''.join(lines)
    schedules.append(schedule)
schedule_indices = {}
duplicate_pair_indices = None

for i, schedule in enumerate(schedules):
    if schedule in schedule_indices:
        duplicate_pair_indices = (schedule_indices[schedule], i)
        break
    else:
        schedule_indices[schedule] = i

print(len(set(schedules)))
# if duplicate_pair_indices is not None:
#     print(
#         f"Duplicate schedules found at {solutions[duplicate_pair_indices[0]]}, {solutions[duplicate_pair_indices[1]]}.")
# else:
